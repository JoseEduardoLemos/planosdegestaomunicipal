...
export default function Cidade({cidade}){
  ...
  {console.log(`hello main ${cidade}`)}
    return (
      <div>
        <NavBar></NavBar>
        <br></br>
        <div id="titulosite">
          <h1 className="titulocidade">Planos de Gestão Para {idCidade}</h1>
  ...
  export async function getServerSideRender(){
    const router = useRouter();
    {console.log(`hello router ${router}`)}
    const idCidade = Number(router);
    {console.log(`hello idCidade ${idCidade}`)}
