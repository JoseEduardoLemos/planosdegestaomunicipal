...
export default function Cidade({planos}){
        ...
        <div className='cardsindex'>
          {
            planos.map(planos =>{
              return(
                <CardMunicipio title={planos.descricao} link ={`/cidades/planos/${planos.idkey}`} imgsrc={planos.url_imagem_plano}/>
              )
            })
          }
        </div>
  ...
  export const getServerSideProps: GetServerSideProps = async (Router) =>{
    ...
    const planos = await prisma.plano.findUnique({where:{idkey:1}});
